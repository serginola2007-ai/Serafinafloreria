...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   - Selector de paleta de colores (.color-opcion)
...
const PedidoForm = (() => {
  ...
  function _setRowError(sumId, hasError) {
    ...
    if (!el) return;
    const row = el.closest('.resumen-item');
    if (!row) return;
    row.classList.toggle('resumen-error', hasError);

    // Inyecta/remueve el mensaje de error inline bajo el placeholder
    const existing = row.querySelector('.resumen-error-mensaje');
    if (hasError && !existing) {
      const msg = document.createElement('span');
      msg.className = 'resumen-error-mensaje';
  ...
  function _clearErrors() {
    ...
    const btn = $('#.resumen-enviar-boton') || document.querySelector('.resumen-enviar-boton');
    btn?.classList.remove('resumen-temblor');
  }
  ...
  function updateSummary() {
    ...
    REQUIRED.forEach(({ key, sumId }) => {
      const row = document.getElementById(sumId)?.closest('.resumen-item');
      if (row?.classList.contains('resumen-error') && state[key]) {
  ...
  function sendToWhatsApp() {
    ...
    if (invalids.length > 0) {
      ...
      const btn = document.querySelector('.resumen-enviar-boton');
      if (btn) {
        btn.classList.remove('resumen-temblor');
        // Forzar reflow para reiniciar la animación si se aprieta dos veces
        void btn.offsetWidth;
        btn.classList.add('resumen-temblor');
        btn.addEventListener('animationend', () => btn.classList.remove('resumen-temblor'), { once: true });
      }

      // Scroll al summary box para que el usuario vea los errores
      const box = document.querySelector('.resumen');
      if (box) {
  ...
  function init() {
    ...
    $$('.chip').forEach(chip => {
      chip.addEventListener('click', () => {
        ...
        $$(`.chip[data-group="${group}"]`).forEach(c => c.classList.remove('seleccionado'));
    ...
    // Colores
    $$('.color-opcion').forEach(opt => {
      opt.addEventListener('click', () => {
        $$('.color-opcion').forEach(o => o.classList.remove('seleccionado'));
...
const FechaValidacion = (() => {
  ...
  function init() {
    ...
    const hint  = document.getElementById('fecha-ayuda');
    if (!input) return;
    ...
    input.addEventListener('change', () => {
      ...
      if (!val) { hint.textContent = ''; hint.className = 'fecha-ayuda'; return; }
      ...
      if (diff < 0) {
        ...
        hint.className = 'fecha-ayuda fecha-ayuda-error';
        input.value = '';
        return;
      }
      ...
      if (diff < 1) {
        ...
        hint.className = 'fecha-ayuda fecha-ayuda-alerta';
      } else if (diff < 2) {
        hint.textContent = '✓ Mañana · ' + _formatDate(sel) + ' — confirmamos disponibilidad.';
        hint.className = 'fecha-ayuda fecha-ayuda-ok';
      } else {
