...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   Botones .filtro-boton con data-filter="all|romanticos|cumpleanos|..."
   Muestra/oculta tarjetas .tarjeta-producto según data-cat.
   ───────────────────────────────────────────────────────────────────────── */

const CatalogoFilter = (() => {
  function init() {
    const btns  = $$('.filtro-boton');
    const cards = $$('.tarjeta-producto');
...
const ShareBtn = (() => {
  function init() {
    $$('.compartir-boton').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        e.preventDefault();
        const card   = btn.closest('.tarjeta-producto');
        const nombre = card?.querySelector('.tarjeta-producto-titulo')?.textContent?.trim() || 'Arreglo floral';
        ...
        try {
          ...
          setTimeout(() => btn.classList.remove('copiado'), 2200);
        } catch (_) {
